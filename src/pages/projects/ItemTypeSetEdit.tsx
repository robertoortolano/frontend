...
export default function ItemTypeSetEdit() {
  const { itemTypeSetId } = useParams<{ itemTypeSetId: string }>();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const itemTypeSetRes = await axios.get(`http://localhost:8080/api/item-type-sets/${itemTypeSetId}`, {
          headers,
        });
            ...
            return (
              <div key={itemTypeId} className={form.inlineGroup}>
                <input
                  ...
                  className={form.input}
                />
                <input type="text" value={category || ""} disabled className={form.input} />
                <button
                  type="button"
