...
export default function FieldSets() {
  ...
  useEffect(() => {
    ...
    const fetchFieldSets = async () => {
      try {
        const url = isProjectContext ? `/field-sets/project/${id}` : "/field-sets";

        const res = await api.get(url, {
          headers: { Authorization: `Bearer ${token}` },
        });

        setFieldSets(res.data);
      } catch (e: any) {
          ...
          <li key={set.id} className={layout.block}>
            <div className={`${layout.blockHeaderButton} flex items-center justify-between`}>
              ...
              <button
                ...
                title={set.defaultFieldSet ? "FieldSet di default non modificabile" : ""}
              >
                ✎ Edit
              </button>
            </div>
  ...
  return (
    <div className={layout.container}>
      <h1 className={layout.title}>Field Sets {isProjectContext ? "di Progetto" : "Globali"}</h1>
