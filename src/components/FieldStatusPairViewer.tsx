...
interface FieldStatusPairViewerProps {
...
interface RoleData {
  ...
  grants?: any[];
  assignmentType?: 'GRANT' | 'ROLE' | 'GRANTS';
}

interface FieldStatusPair {
...
export default function FieldStatusPairViewer({ itemTypeSetId }: FieldStatusPairViewerProps) {
  const [fieldStatusPairs, setFieldStatusPairs] = useState<FieldStatusPair[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [expandedPairs, setExpandedPairs] = useState<Record<string, boolean>>({});
  const [selectedRoleForGrants, setSelectedRoleForGrants] = useState<RoleData | null>(null);
  ...
  const fetchFieldStatusPairs = async () => {
    try {
      ...
      const pairsMap = new Map<string, FieldStatusPair>();
      
      [...editorRoles.data, ...viewerRoles.data].forEach((role: RoleData) => {
        ...
        if (role.roleType === 'EDITOR') {
          pair.editors.push(role);
        } else if (role.roleType === 'VIEWER') {
          pair.viewers.push(role);
        }
      });
